...
test('Update profile with new password without number', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'mật khẩu mới', exact: true }).fill('abcdefG@');
  
  await page.getByRole('textbox', { name: 'Xác nhận mật khẩu mới' }).click();
  await page.getByRole('textbox', { name: 'Xác nhận mật khẩu mới' }).fill('abcdefG@');
