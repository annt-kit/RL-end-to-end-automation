...
test('Update profile with new password without letter', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'mật khẩu mới', exact: true }).fill('1234567@');

  await page.getByRole('textbox', { name: 'Xác nhận mật khẩu mới' }).click();
  await page.getByRole('textbox', { name: 'Xác nhận mật khẩu mới' }).fill('1234567@');
