...
test('Update profile with new password without uppercase letter', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'mật khẩu mới', exact: true }).fill('12345ab@');

  await page.getByRole('textbox', { name: 'Xác nhận mật khẩu mới' }).click();
  await page.getByRole('textbox', { name: 'Xác nhận mật khẩu mới' }).fill('12345ab@');
