...
test('Update profile with new password without special character', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'mật khẩu mới', exact: true }).fill('123456ab');

  await page.getByRole('textbox', { name: 'Xác nhận mật khẩu mới' }).click();
  await page.getByRole('textbox', { name: 'Xác nhận mật khẩu mới' }).fill('123456ab');
