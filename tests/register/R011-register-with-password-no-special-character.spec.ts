...
test('Register with password that has no special character', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Mật khẩu', exact: true }).fill('123456aA');
  await page.getByRole('textbox', { name: 'Xác nhận mật khẩu' }).click();
  await page.getByRole('textbox', { name: 'Xác nhận mật khẩu' }).fill('123456aA');
