...
test('Register with password that has no number', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Mật khẩu', exact: true }).fill('abcdefg@');
  await page.getByRole('textbox', { name: 'Xác nhận mật khẩu' }).click();
  await page.getByRole('textbox', { name: 'Xác nhận mật khẩu' }).fill('abcdefg@');
