...
test('Register with password that has no lowercase letter', async ({ page }) => {
  ...
  await page.getByLabel('Sinh viên').getByText('Sinh viên').click();
  ...
  await page.getByRole('textbox', { name: 'Mật khẩu', exact: true }).fill('123456A@');
  await page.getByRole('textbox', { name: 'Xác nhận mật khẩu' }).click();
  await page.getByRole('textbox', { name: 'Xác nhận mật khẩu' }).fill('123456A@');
