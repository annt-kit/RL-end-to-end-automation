...
test('Register with password that has no letter', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Mật khẩu', exact: true }).fill('1234567@');
  await page.getByRole('textbox', { name: 'Xác nhận mật khẩu' }).click();
  await page.getByRole('textbox', { name: 'Xác nhận mật khẩu' }).fill('1234567@');
